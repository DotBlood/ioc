...
func WallRun(ctx context.Context, e *engine.Engine, spec *WallSpec, packetsW, goldW io.Writer, mode core.QueryMode, hierarchical, collapsed bool, coarseK int, rerank bool, graphBoost float64, importanceWeight float64) (*WallReport, error) {
	topK := spec.TopK
	if topK <= 0 {
		topK = 5
	}
	...
	for _, q := range spec.Questions {
		...
		_, hits, err := e.Query(ctx, core.Query{
			...
			GraphBoost: graphBoost, ImportanceWeight: importanceWeight,
		})
		if err != nil {
			return nil, fmt.Errorf("question %q: %w", q.ID, err)
		}
