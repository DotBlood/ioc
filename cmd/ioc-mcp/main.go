...
func (a *ioc) register(s *server.MCPServer) {
	...
	s.AddTool(mcp.NewTool("ioc_query",
		...
		mcp.WithNumber("graph_synonym", mcp.Description("opt-in synonym-edge cosine threshold for graph-boost PPR (0=off; densifies the walk graph with encoder-cosine links, no LLM)")),
	), a.query)
