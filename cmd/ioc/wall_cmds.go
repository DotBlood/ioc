...
func runWall(args []string) int {
	...
	graphSynonym := fs.Float64("graph-synonym", 0, "opt-in synonym-edge cosine threshold for graph-boost PPR (0=off)")
	...
	rep, err := eval.WallRun(context.Background(), e, spec, pf, gf, qm, hier, coll, *coarseK, *rerank, *graphBoost, *graphSynonym)
	if err != nil {
		fmt.Fprintln(os.Stderr, "error: wall run:", err)
		return 1
	}
	fmt.Printf("config: mode=%s coarsek=%d rerank=%v graph-boost=%g graph-synonym=%g\n", *mode, *coarseK, *rerank, *graphBoost, *graphSynonym)
