...
| **Graph-aware retrieval v1** — `Query.GraphBoost` (opt-in, OFF by default; reorder-only, `Hit.Score` stays cosine, no-op with no edges → proven default path byte-for-byte untouched). CLI `query/wall -graph-boost`; MCP `graph_boost`. **Honest caveat (2026-06-06 experiment, WALL_EXPERIMENT.md):** v1 does NOT yet add value — it is centrality-biased (lifts global hubs, not query-anchored neighbours) and noisier than `Related`; a blind agent independently called it "a weaker proxy". Stays OFF; the structural win is the explicit `Related` edge-walk. | SHIPPED (off; not yet useful) | `internal/engine` (`blendGraph`) |
...
| **Graph-aware retrieval v2 (redesign)** | The 2026-06-06 experiment showed v1's `graph-boost` is centrality-biased and does not help. v2 must **anchor the boost to the query's strong hits** (personalized-PageRank seeded at the top-cosine candidates) and/or **degree-normalize** `g`, so it rewards "connected to what the query matched", not "globally well-connected". Then re-run the edged-corpus experiment. Until then, prefer the `Related` edge-walk. (Deferred extras: kind/direction-filtered boost, recall expansion, rerank composition.) | code (`internal/engine`) + eval |
