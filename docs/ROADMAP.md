...
| **Collapsed-tree retrieval (RESEARCH_ROADMAP R1)** — `Query.Collapsed`: flat over the visible set ∪ ALL descendant-scope artifacts in one pass (no coarse→fine routing, no rollups needed). **Now the user-facing default** (`ioc query`, MCP `ioc_query`) — it strictly dominates plain flat (0.85 vs 0.00 on tree; ties on distinctive) and beats hierarchical on tree (0.85 vs 0.74); hierarchical stays opt-in for dense corpora (0.96). Engine zero-value stays flat. | DONE (default) | `internal/engine` (`collapsedCandidates`), `WALL_EXPERIMENT.md` |
...
| Item | What | Source |
|------|------|--------|
| **Auto mode-selector (collapsed shipped — see DONE)** | Collapsed-tree is now the default (R1, validated). Remaining refinement: auto-pick **hierarchical** when the corpus is dense/near-duplicate (where it still wins 0.96 vs 0.85), e.g. key on rollup density / cluster count — instead of the caller choosing `-mode`. | [`RESEARCH_ROADMAP.md`](RESEARCH_ROADMAP.md) R1, [`WALL_EXPERIMENT.md`](WALL_EXPERIMENT.md) |
