...
| **Graph-aware retrieval** | edge-boosted `Query`: after vector retrieval, pull/boost artifacts connected by edges to the top hits (fuse the structural axis into the semantic one). Currently edges are a separate axis (`Related`); this would blend them. Don't touch the hot Query path until edges are validated in use. | code (`internal/engine`) |
