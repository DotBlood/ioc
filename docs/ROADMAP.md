...
| Item | What | Source |
|------|------|--------|
| **Auto mode-selector** | Collapsed-tree is now the default (R1). Remaining refinement: auto-pick **hierarchical** when the corpus is dense/near-duplicate (where it still wins 0.96 vs 0.85), e.g. key on rollup density / cluster count — instead of the caller choosing `-mode`. | [`WALL_EXPERIMENT.md`](WALL_EXPERIMENT.md) |
